...
export default () => {
  return (state, action) => {
    ...
    const isPendingAction = action.type === actionTypes.CREATE_RESOURCES_PENDING ||
    ...
    const isSucceededAction = action.type === actionTypes.CREATE_RESOURCES_SUCCEEDED ||
    ...
    const isFailedAction = action.type === actionTypes.CREATE_RESOURCES_FAILED ||
    ...
    const isIdleAction = action.type === actionTypes.CREATE_RESOURCES_IDLE ||
    ...
    // for us to update.
    if (!isPendingAction && !isSucceededAction && !isFailedAction && !isIdleAction) {
      return state;
    }
    ...
    if (isPendingAction) {
      ...
      if (action.resourceName && action.resources) {
        newRequest.resources[action.resourceName] = action.resources.map(resource => {
